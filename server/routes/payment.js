...
// WHY: frontend fetches live prices from server
//      displayPrice already has ₹ symbol — eg "₹199"
router.get('/plans', (req, res) => {
    const plans = Object.entries(PLANS).map(([key, val]) => ({
        key,
        amount: val.amount,
        price: val.displayPrice,   // WHY: "₹199" — ready to render
...
router.post('/create-order', requireAuth, paymentLimiter, async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({
            ...
            ...result,
            plan,
            // WHY: Key ID safe to send to frontend — never send Key Secret
...
//      verifies signature + unlocks Pro
