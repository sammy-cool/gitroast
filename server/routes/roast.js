...
// ─── GET /api/roast/stats ─────────────────────────────────
// WHY: landing page fetches real roast count for social proof
//      fake hardcoded number = hurts trust
//      real number, even if small, = authentic
router.get("/stats", async (req, res) => {
  try {
    ...
    // WHY: stats failing should never crash the page
    return res.status(200).json({
      success: true,
      totalRoasts: 0,
    });
